...
export default async function SitePage({
  ...
  const activeContacts = sellers
    .filter((seller) => seller.active && seller.phoneE164)
    .map((seller) => ({
      id: seller.id,
      name: seller.name,
      phoneE164: seller.phoneE164,
      waPreset: seller.waPreset ?? undefined,
    }));
  const fallbackContacts = !activeContacts.length && ownerPhone
    ? [
        {
          id: "owner-contact",
          name: ownerName,
    ...
    : [];
  const contacts = activeContacts.length ? activeContacts : fallbackContacts;
              ...
              {contacts.length ? (
                <ul className="mt-4 space-y-3">
                  {contacts.map((contact) => {
                    const href = waHref(contact.phoneE164, contact.waPreset);
                    const formattedPhone = formatPhoneDisplay(contact.phoneE164) || contact.phoneE164;
                    return (
                      <li key={contact.id}>
                        <a
                          className="btn-secondary w-full flex-col items-start gap-1 text-left"
                          href={href}
                          target="_blank"
                          rel="noopener noreferrer"
                        >
                          <span className="text-sm font-semibold text-slate-900">Escribir a {contact.name}</span>
                          <span className="text-xs font-medium text-slate-500">{formattedPhone}</span>
                        </a>
                      </li>
                    );
                  })}
