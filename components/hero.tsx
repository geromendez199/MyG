        ...
        <div className="relative mx-auto w-full max-w-sm">
          ...
          <div className="relative overflow-hidden rounded-3xl border border-slate-100 bg-white/90 p-6 shadow-2xl">
            ...
            <ul className="mt-4 space-y-4 text-sm text-slate-600">
              {/* Itinerario demo para recordar que falta completar unidades reales. */}
              {[1, 2, 3].map((item) => (
                <li key={item}>
                  <p className="font-semibold text-slate-900">Unidad demo #{item}</p>
                  <p>Completá marca, año y precio reales desde el panel.</p>
                </li>
              ))}
            </ul>
            <p className="mt-6 rounded-2xl bg-primary/5 px-4 py-3 text-xs text-slate-500">
              * Publicá tus vehículos reales para reemplazar este resumen de ejemplo.
            </p>
          </div>
        </div>
      </div>
